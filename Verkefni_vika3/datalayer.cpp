...
vector<Connections> Datalayer::pullConnections()
{
    ...
    if(query.exec("SELECT c.id, p.name AS S_Name, cp.name AS CP_Name FROM persons p JOIN connections c ON p.id = c.pId,computers  cp ON cp.id = c.cId ORDER BY p.id"))
    {
        while(query.next())
        {
           int id = query.value("id").toUInt();
